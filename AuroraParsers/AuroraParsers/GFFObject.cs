...
namespace AuroraParsers.AuroraParsers
{
    public class GFFObject
    {
            ...
            for (int i = 0; i != FieldArray.Count - 1; i++)
            {
                ...
                switch (_Field.Type)
                {
                    ...
                    case (int)DataTypes.DWORD64:
                        ...
                        ulong DWORD = ulong.Parse(_Field.Value); //Convert the data string back to a DWORD
                    ...
                    case (int)DataTypes.INT64:
                        ...
                        long _INT64 = long.Parse(_Field.Value); //Convert the data string back to a INT64
                    ...
                    case (int)DataTypes.CExoString:
                        ...
                        byte[] CExoStringsize = BitConverter.GetBytes((uint)CExoStringBytes.Length); //Get the byte size of the CExoString 4 Bytes long
        ...
        private byte[] GetCExoLocString(int offset)
        {
            ...
            int length = Reader.ReadInt32();//Total Size of the GetCExoLocString
        ...
        private string GetCExoString(int offset)
        {
            ...
            int length = Reader.ReadInt32();// Get the length of the string
            if (length != 0)
            {
                ResRef = new string(Reader.ReadChars(length));
            }
