...
//         let min_data_size =  get_min_data_size(is_uncompressed, encoding)?;
//         self.format = get_pixel_format(is_uncompressed, encoding)?;
...
//             complete_data_size += get_data_size(self.format, width, height)?;
//         }
...
//         self.is_animated =  check_animated(width as u32, height as u32, data_size)?;
...
//         if !has_valid_dimensions(self.format, width as i32, height as i32) {
//             return Err(format!(
...
//              decompress_mipmap(&mut decompressed, mip_map, self.format)?;
